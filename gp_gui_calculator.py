...
📐 Fermat
═══════════════════════════════════════════════════════════════
• Fermat'scher Kleine Satz: Mod(a, p)^(p-1) == 1 (für Primzahl p)
• Beispiel: Mod(3, 7)^6 == 1
• Test: isprime(p) um zu prüfen ob p eine Primzahl ist
• Bestimmung Fermat-Lügner: n= sollte vorgegben sein, Ausgabe der Lösung in der Konsole von PyCharm
...
• Primfaktorzerlegung: factor(123456789)
  ...
  Grösstmögliches Element welches Erzeugendes ist(bei p, die richitge zahl eingeben, 467 ist bsp..): p=467; g=p-1; while(znorder(Mod(g,p))!=p-1, g--); g
  Kleinstmögliches Element welches Erzeugendes ist(bei p, die richitge zahl eingeben, 467 ist bsp..): p=467; F=factor(p-1)[,1]; g=2; while(sum(i=1,#F, Mod(g,p)^((p-1)/F[i])==1), g++); g
  Ordnung jedes Elements (alle Einheiten) ausgeben, n anpassen: 
  
  n=14;
for(a=1, n-1,
  if(gcd(a,n)==1,
    print(a, " -> Ordnung ", znorder(Mod(a,n)))
  )
);
...
Beispiel 9 - Ordnung von G in Zp (p und g anpassen):
p = 71;
g = 51;
znorder(Mod(g,p))

Beispiel 9 - Untergruppen (der von 2 in Z*₁₇ erzeugten Gruppe):
